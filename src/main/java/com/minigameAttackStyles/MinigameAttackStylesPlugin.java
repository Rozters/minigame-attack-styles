...
import static com.minigameAttackStyles.AttackStyle.CASTING;
import static com.minigameAttackStyles.AttackStyle.DEFENSIVE;
import static com.minigameAttackStyles.AttackStyle.DEFENSIVE_CASTING;
import static com.minigameAttackStyles.AttackStyle.OTHER;
import net.runelite.client.ui.overlay.OverlayManager;
import java.util.HashSet;

@Slf4j
@PluginDescriptor(
	name = "Minigame Attack Styles",
	description = "Enable warned attack styles within minigames with disabled exp drops.",
	tags = {"combat", "defence", "magic", "overlay", "ranged", "strength", "warn", "pure", "minigames"}
)
public class MinigameAttackStylesPlugin extends Plugin
{
	private int gitequippedWeaponTypeVarbit = -1;
	...
	private static final Map<MinigameTypes, Set<Integer>> REGION_MAP = new HashMap<>();
	private boolean wasInMinigame = false;

	static
	{
		for (MinigameTypes type : MinigameTypes.values())
		{
			REGION_MAP.put(type, new HashSet<>());
		}

		for (MinigameRegion region : MinigameRegion.values())
		{
			REGION_MAP.get(region.type).addAll(region.regionSet);
		}
	}
	...
	@Inject
	private MinigameAttackStylesConfig config;

	@Inject
	private OverlayManager overlayManager;

	@Inject
	private MinigameAttackStylesOverlay overlay;
	...
	@Provides
	MinigameAttackStylesConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(MinigameAttackStylesConfig.class);
	}
	...
	{
		if (event.getGroup().equals("minigame attack styles"))
		{
	...
	{
		boolean isInMinigame = isInMinigame(MinigameTypes.CLAN_WARS)
				|| isInMinigame(MinigameTypes.LAST_MAN_STANDING)
				|| isInMinigame(MinigameTypes.BOUNTY_HUNTER);

		if (isInMinigame != wasInMinigame)
		{
			updateWarning();
			processWidgets();
		}
	...
	// Check if user is in a minigame
	private boolean isInMinigame(MinigameTypes type)
	{
		Set<Integer> regions = REGION_MAP.get(type);
	...
	private boolean shouldIgnoreWarnings()
	{
		// LMS → always ignore if enabled
		if (config.ignoreForLMS() && isInMinigame(MinigameTypes.LAST_MAN_STANDING))
		{
			return true;
		}

		// Bounty Hunter → always ignore if enabled
		if (config.ignoreForBH() && isInMinigame(MinigameTypes.BOUNTY_HUNTER))
		{
			return true;
		}

		// Clan Wars → ONLY ignore if XP is disabled
		if (config.ignoreForClanWarsIfPossible()
				&& isInMinigame(MinigameTypes.CLAN_WARS)
				&& client.getVarbitValue(VarbitID.CLAN_WARS_XP_DISABLE) == 1)
		{
			return true;
		}
	...
	private void updateWarning()
	{
		if (shouldIgnoreWarnings())
		{
			...
			// also ensure nothing gets hidden
			updateWidgetsToHide(false);
			return;
		}
